...
export default function Admin() {
  ...
  const tabs = [
    ...
    { label: "Mídia", href: "/admin/midia", icon: FileCode },
  ...
  return (
    <div className="pt-24 min-h-screen bg-background flex">
      {/* Sidebar Desktop */}
      <aside className="sticky top-24 hidden h-[calc(100vh-96px)] w-[350px] shrink-0 overflow-y-auto border-r border-white/5 bg-surface/50 md:block">
        <div className="p-6">
          <p className="text-[10px] font-bold text-primary uppercase tracking-[0.3em] mb-8">
            Gestão Premium
          </p>
...
function AdminDashboard() {
  ...
  async function fetchDashboardData() {
    ...
    try {
      ...
      const allLeads = cSnap.docs.map(
        (d) => ({ id: d.id, ...d.data() }) as any,
      );
      const filteredLeads = allLeads.filter((l) =>
        isAfter(new Date(l.createdAt || 0), filterDate),
      );

      const allVisits = visitSnap.docs.map((d) => d.data());
      const filteredVisits = allVisits.filter((v) =>
        isAfter(new Date(v.createdAt || 0), filterDate),
      );
  ...
  const handleSeed = async () => {
    ...
    try {
      ...
      await Promise.all(
        brandsList.map((b) => addDoc(collection(db, "brands"), { name: b })),
      );
      ...
      const vData = [
        {
          ...
          kilometers: 150000,
          imageUrl:
            "https://images.unsplash.com/photo-1586191582151-f73770425983?q=80&w=800",
        },
        {
          brand: "Volvo",
          model: "FH 540",
        ...
        },
        {
          ...
          kilometers: 210000,
          imageUrl:
            "https://images.unsplash.com/photo-1591768793355-74d7c836038c?q=80&w=800",
        },
      ];
    ...
    <div className="space-y-12 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="text-on-surface-variant uppercase tracking-[0.3em] text-[10px] font-bold">
            Painel de Analítica Premium Caminhões
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          ...
          <div
            key={i}
            className="bg-surface p-6 border border-white/5 relative overflow-hidden group"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-surface border border-white/5 p-8">
          <div className="flex justify-between items-center mb-8">
            <h3 className="text-white font-headline font-black uppercase text-sm tracking-widest border-l-4 border-primary pl-4">
              Fluxo de Leads
            </h3>
        ...
        <div className="bg-surface border border-white/5 p-8">
          <h3 className="text-white font-headline font-black uppercase text-sm tracking-widest border-l-4 border-primary pl-4 mb-8">
            Top Origens (Leads)
          </h3>
              ...
              <div key={loc.name}>
                ...
                <div className="h-1 bg-white/5 w-full overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{
                      width: `${(loc.value / (stats.leads || 1)) * 100}%`,
                    }}
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-white/10">
                <th className="px-8 py-4 text-[10px] font-bold text-primary uppercase tracking-widest">
                  Lead
                </th>
                <th className="px-8 py-4 text-[10px] font-bold text-primary uppercase tracking-widest">
                  WhatsApp
                </th>
                <th className="px-8 py-4 text-[10px] font-bold text-primary uppercase tracking-widest text-right">
                  Data
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5">
                ...
                >
                  <td className="px-8 py-4">
                    <div className="flex flex-col">
                      <span className="text-white font-headline font-bold uppercase text-xs">
                        {lead.name}
                      </span>
                      <span className="text-[10px] text-on-surface-variant tracking-widest font-body line-clamp-1">
                        {lead.message}
                      </span>
                    </div>
                  </td>
                  <td className="px-8 py-4 text-[10px] font-body text-on-surface-variant font-bold">
                    {lead.whatsapp}
                  </td>
                  <td className="px-8 py-4 text-[10px] font-body text-white/40 text-right">
                    {format(new Date(lead.createdAt || 0), "dd MMM, HH:mm", {
                      locale: undefined,
                    })}
                  </td>
                </tr>
              ...
              {stats.recentLeads.length === 0 && (
                <tr>
                  <td
                    colSpan={3}
                    className="px-8 py-10 text-center text-on-surface-variant text-[10px] uppercase font-bold tracking-widest"
                  >
  ...
  return (
    <div className="space-y-12">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-4xl font-headline font-black text-white uppercase tracking-tighter mb-2">
            Gestão de Veículos
          </h1>
          <p className="text-on-surface-variant uppercase tracking-widest text-xs font-bold">
            Adicione e gerencie o estoque premium.
          </p>
        </div>
        <button
          onClick={() => {
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="bg-background border-b border-white/10">
              <th className="px-6 py-4 font-headline uppercase text-xs tracking-widest text-primary">
                Marca / Modelo
              </th>
              <th className="px-6 py-4 font-headline uppercase text-xs tracking-widest text-primary">
                Preço
              ...
              <th className="px-6 py-4 font-headline uppercase text-xs tracking-widest text-primary text-right">
                Ações
              </th>
            </tr>
          </thead>
            ...
            {vehicles.map((v) => (
              <tr key={v.id} className="hover:bg-white/5 transition-colors">
                <td className="px-6 py-6 font-headline font-bold uppercase text-sm text-white">
                  {v.brand} {v.model}{" "}
                  <span className="block text-[10px] text-on-surface-variant font-normal tracking-widest mt-1">
                    Ano {v.year}
                  </span>
                </td>
                <td className="px-6 py-6 font-headline font-bold text-lg text-primary">
                ...
                <td className="px-6 py-6">
                  {v.isFeatured ? (
                    ...
                    <span className="text-on-surface-variant font-bold text-[10px] uppercase tracking-widest">
                      Não
                    </span>
                  )}
                </td>
            ...
            >
              <div className="space-y-4">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold uppercase text-primary tracking-widest">
                    Marca*
                  </label>
                  <input
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full bg-[#2a2a2a] border-none text-white py-3 px-4 outline-none mb-2"
                  />
                  <MediaUpload
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold uppercase text-primary tracking-widest">
                    Imagens Adicionais (Galeria) - Uma URL por linha
                  </label>
                  <textarea
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <input
                      {...register("power")}
                      defaultValue={editingVehicle?.power}
                      className="w-full bg-[#2a2a2a] border-none text-white py-3 px-4 outline-none"
                      placeholder="Ex: 540 CV"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold uppercase text-primary tracking-widest">
                ...
                <div className="space-y-1">
                  ...
                  <textarea
                    ...
                    className="w-full bg-[#2a2a2a] border-none text-white py-3 px-4 outline-none h-24 resize-none"
                  />
                </div>
                <div className="flex items-center gap-3 py-2">
                  <input
                    type="checkbox"
                    {...register("isFeatured")}
                    defaultChecked={editingVehicle?.isFeatured}
                    id="isFeatured"
                    className="w-5 h-5 accent-primary"
                  />
                  <label
                    htmlFor="isFeatured"
                    className="text-xs uppercase font-bold text-white cursor-pointer tracking-widest"
                  >
                    Destacar na Home
                  </label>
                </div>
              ...
              <div className="md:col-span-2 pt-6">
                <button
                  type="submit"
                  className="w-full industrial-gradient text-black py-5 font-headline font-black uppercase tracking-[0.2em]"
                >
  ...
  return (
    <div className="space-y-12">
      <div>
        ...
        <p className="text-on-surface-variant uppercase tracking-widest text-xs font-bold">
          Gerencie os leads vindos do formulário de contato.
        </p>
      </div>
      ...
      <div className="bg-surface border border-white/5 overflow-x-auto">
          ...
          <tbody className="divide-y divide-white/5">
            {messages.map((m) => (
              ...
              >
                ...
                <td className="px-6 py-6 font-headline font-bold uppercase text-sm text-white">
                  {m.name}
                  <p className="text-[10px] font-normal text-on-surface-variant italic normal-case mt-1 max-w-md line-clamp-2">
                    "{m.message}"
                  </p>
                </td>
                <td className="px-6 py-6 text-xs text-on-surface-variant space-y-1">
                  <p className="flex items-center gap-2">
                    <Phone size={12} className="text-primary" /> {m.whatsapp}
                  </p>
                  <p className="flex items-center gap-2">
                    <Mail size={12} className="text-primary" /> {m.email}
                  </p>
                </td>
                <td className="px-6 py-6">
                  <select
                ...
                <td className="px-6 py-6 text-right space-x-3">
                  ...
                  <button
                    ...
                    className="text-red-500 opacity-0 group-hover:opacity-100 transition-opacity transition-all"
                  >
                    <Trash2 size={18} />
                  </button>
                </td>
        ...
        {messages.length === 0 && !loading && (
          <div className="p-20 text-center text-on-surface-variant uppercase text-xs tracking-widest font-bold">
  ...
  return (
    <div className="space-y-12">
      <div>
        ...
        <p className="text-on-surface-variant uppercase tracking-widest text-xs font-bold">
          Personalize a identidade visual e informações do site.
        </p>
      </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
          <div className="space-y-4">
            <div className="space-y-1">
              ...
              <p className="text-[10px] text-on-surface-variant mt-2 italic">
                Recomendado: Fundo transparente (PNG), altura mínima de 128px.
              </p>
            </div>
          </div>
          ...
          {settings?.logoUrl && (
            <div className="p-4 bg-background border border-white/5 inline-block">
              <p className="text-[10px] font-bold text-primary uppercase mb-4 tracking-widest">
                Pré-visualização:
              </p>
              <img
    ...
    <div className="space-y-12">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-on-surface-variant uppercase tracking-widest text-xs font-bold">
            Gerencie os slides da home page.
          </p>
        </div>
        <button
          onClick={() => {
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {banners.map((b) => (
            <div
              key={b.id}
              className="bg-surface border border-white/5 overflow-hidden group hover:border-primary/50 transition-all"
            >
              ...
              <div className="p-4 flex justify-between items-center bg-surface">
                <span className="text-white font-headline font-bold uppercase text-xs truncate max-w-[150px]">
                  {b.title || "Sem título"}
                </span>
          ...
          {banners.length === 0 && (
            <div className="col-span-full py-20 text-center text-on-surface-variant uppercase text-xs font-bold tracking-widest">
              Nenhum banner cadastrado no banco de dados. Use o botão no
              dashboard para carregar iniciais.
            </div>
          )}
        </div>
      )}
      ...
      {isFormOpen && (
        <div className="fixed inset-0 bg-black/90 z-[100] flex items-center justify-center p-6">
          <div className="bg-surface w-full max-w-lg p-10 border border-white/10 relative">
            ...
            <h2 className="text-xl font-headline font-black text-white uppercase mb-8 border-l-4 border-primary pl-4">
              Gestão de Banner
            </h2>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full bg-[#2a2a2a] text-white py-3 px-4 outline-none mb-2"
                />
                <MediaUpload
              ...
              <button
                type="submit"
                className="w-full industrial-gradient text-black py-4 font-headline font-black uppercase tracking-widest hover:scale-[1.02] transition-all"
              >
                Salvar
              </button>
            </form>
...
function MediaUpload({
  ...
  return (
    <div className="space-y-1">
      <label className="text-[10px] font-bold uppercase text-primary tracking-widest block mb-2">
        {label}
      </label>
    ...
    <div className="space-y-12">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-on-surface-variant uppercase tracking-widest text-xs font-bold">
            Defina as marcas disponíveis para filtro.
          </p>
        </div>
        <button
          onClick={() => setIsFormOpen(true)}
          className="industrial-gradient text-black font-headline font-black uppercase text-xs px-8 py-4 flex items-center gap-3 transition-all hover:scale-105"
        >
      ...
      <div className="flex flex-wrap gap-4">
        {brands.map((b) => (
          <div
            key={b.id}
            className="bg-surface border border-white/5 px-6 py-4 flex items-center justify-between gap-6 group hover:border-primary/30 transition-all"
          >
            <span className="text-white font-headline font-bold uppercase tracking-widest">
              {b.name}
            </span>
            <button
              onClick={() => handleDelete(b.id)}
        ...
        {brands.length === 0 && !loading && (
          <div className="text-on-surface-variant uppercase text-xs font-bold tracking-widest">
        ...
        <div className="fixed inset-0 bg-black/90 z-[100] flex items-center justify-center p-6">
          <div className="bg-surface w-full max-w-sm p-10 border border-white/10 relative">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <button
                type="submit"
                className="w-full industrial-gradient text-black py-4 font-headline font-black uppercase tracking-widest hover:scale-105 transition-all"
              >
                Adicionar
              </button>
            </form>
...
function MediaManager() {
  const [media, setMedia] = useState<{id: string, url: string, name: string, createdAt: number}[]>([]);
  const [loading, setLoading] = useState(true);
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Excluir esta mídia? Ela pode estar sendo usada no site.")) return;
    try {
  ...
  return (
    <div className="space-y-12">
      <div>
        ...
        <p className="text-on-surface-variant uppercase tracking-widest text-xs font-bold">
          Suba suas imagens aqui para gerar links rápidos e colar no site.
        </p>
      </div>

      <div className="bg-surface p-8 border border-white/5 max-w-xl">
        <MediaUpload 
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {media.map((item) => (
            <div key={item.id} className="bg-surface border border-white/5 group hover:border-primary/50 transition-all overflow-hidden flex flex-col">
              <div className="aspect-square relative overflow-hidden bg-black/20">
                ...
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                    <button 
                        ...
                        className="bg-primary text-black p-2 rounded-full hover:scale-110 transition-transform"
                        title="Copiar Link"
                    >
              ...
              <div className="p-3">
                  <p className="text-[10px] text-white font-bold uppercase truncate mb-2">{item.name}</p>
                  <button 
                    onClick={() => copyToClipboard(item.url, item.id)}
                    className="w-full bg-white/5 hover:bg-white/10 py-1 text-[8px] uppercase tracking-widest font-black text-primary transition-colors flex items-center justify-center gap-2"
                  >
